...
static void handle_up(t_shell *shell, char *str)
{
	size_t comm_len;
...
static void handle_down(t_shell *shell, char *str)
{
	size_t comm_len;
...
static void handle_del(t_shell *shell, char *str)
{
	char *tmp;
...
static void handle_execute(t_shell *shell, char **envp, char **argv, char *str)
{
	char *tmp;
...
static void handle_read(t_shell *shell, char **envp, char **argv, ssize_t read_len, char *str)
{
	...
	if (!strcmp(str, LEFT) || !strcmp(str, RIGHT))
		...
		handle_down(shell, str);
	else if (!strcmp(str, DEL))
		handle_del(shell, str);
	else if (!strcmp(str, "\n"))
		handle_execute(shell, envp, argv, str);
	else
	{
...
static void ft_readline(char **envp, char **argv, t_shell *shell)
{
	...
	while (strcmp(str, CTRLD))
	{
		...
		if (was_signal && shell->hist_curr->len)
		{
			tmp = shell->hist_curr->command;
			init_hist(shell);
			free(tmp);
